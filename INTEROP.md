...
How **attestport** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `attestport` lives in the **Network / recon** cluster.
...
| from | relation | to |
|---|---|---|
| `attestport` | JSON findings compose with | [`nmapparse`](https://github.com/cognis-digital/nmapparse), [`pcapview`](https://github.com/cognis-digital/pcapview), [`dnsrecon`](https://github.com/cognis-digital/dnsrecon), [`subhunt`](https://github.com/cognis-digital/subhunt) |
| `attestport` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `attestport` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `attestport` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the network / recon cluster.** `attestport` output feeds the next tool:
```bash
attestport ... --format json > out.json          # this tool's findings (see `attestport --help`)
...
attestport ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
attestport ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
attestport ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `attestport`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `attestport` slots
